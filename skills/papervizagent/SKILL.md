...
`papervizagent models --json '<arguments>'` prints a JSON result. MCP tool `models` on server `papervizagent` returns the same result as `structuredContent`.
...
`papervizagent web --json '<arguments>'` prints a JSON result. MCP tool `web` on server `papervizagent` returns the same result as `structuredContent`.
