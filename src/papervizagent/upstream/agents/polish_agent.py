...
def _load_image_as_base64(image_path: str) -> str:
    ...
    try:
        ...
        _stderr_print(f"❌ Error loading image {image_path}: {e}")
        return None
...
class PolishAgent(BaseAgent):
    ...
    async def _generate_suggestions(self, gt_image_b64: str, style_guide: str) -> str:
        ...
        try:
            ...
            _stderr_print(f"❌ Error during suggestion generation: {e}")
            return ""
    ...
    async def process(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not gt_image_path_rel:
            _stderr_print(f"⚠️  No GT image path found in data")
            return data
        ...
        if not gt_image_b64:
            _stderr_print(f"⚠️  Failed to load GT image from {gt_image_path}")
            return data
        
        # Load style guide
        style_guide_path = self.exp_config.resource_path("style_guides", self.style_guide_filename)
        try:
            with open(style_guide_path, "r", encoding="utf-8") as f:
                style_guide = f.read()
        except Exception as e:
            _stderr_print(f"❌ Error loading style guide from {style_guide_path}: {e}")
            return data
            
        _stderr_print(f"🎨 [Step 1] Generating suggestions for {task_name}...")
        ...
        if not suggestions or "No changes needed" in suggestions:
            _stderr_print(f"✨ No changes needed for this image.")
        ...
        _stderr_print(f"📝 Suggestions: {suggestions[:200]}...")
        
        # Step 2: Polish Image using suggestions
        _stderr_print(f"🎨 [Step 2] Polishing image with suggestions...")
        ...
        try:
            ...
            if response_list and response_list[0]:
                ...
                if converted_jpg:
                    ...
                    _stderr_print(f"⚠️  Image conversion failed")
            else:
                _stderr_print(f"⚠️  No response from model")
                
        except Exception as e:
            _stderr_print(f"❌ Error during image generation: {e}")
