...
class CriticAgent(BaseAgent):
    ...
    async def process(self, data: Dict[str, Any], source: str = "stylist") -> Dict[str, Any]:
        ...
        if image_base64 and len(image_base64) > 100:
            ...
            _stderr_print(f"⚠️ [Critic] No valid image found for round {round_idx}. Using text-only critique mode.")
            content_list.append({
                "type": "text", 
        ...
        try:
            ...
            critic_suggestions = eval_result["critic_suggestions"]
            revised_description = eval_result["revised_description"]
            if not isinstance(critic_suggestions, str) or not isinstance(revised_description, str):
                raise ValueError("critic fields must be strings")
        except Exception as e:
            data[f"target_{task_name}_critic_error{round_idx}"] = str(e)
            return data
