...
class RetrieverAgent(BaseAgent):
    ...
    async def process(self, data: Dict[str, Any], retrieval_setting: str = "auto") -> Dict[str, Any]:
        ...
        if retrieval_setting in ["auto", "random"] and not ref_file.exists():
            _stderr_print(f"Warning: Reference file not found at {ref_file}. Falling back to retrieval_setting='none'.")
        ...
        if retrieval_setting == "manual":
            ...
            if not manual_file.exists():
                _stderr_print(f"Warning: Manual reference file not found at {manual_file}. Falling back to retrieval_setting='none'.")
                retrieval_setting = "none"
    ...
    def _parse_retrieval_result(self, raw_response: str, task_name: str) -> list:
        ...
        """
        try:
            parsed = json.loads(raw_response.removeprefix("```json").removesuffix("```").strip())
            ...
            _stderr_print(f"Warning: Failed to parse retrieval result: {e}")
            _stderr_print(f"Raw response: {raw_response[:200]}...")
            return []
