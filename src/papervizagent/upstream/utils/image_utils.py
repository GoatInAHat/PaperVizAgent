...
def convert_png_b64_to_jpg_b64(png_b64_str: str) -> str:
    ...
    try:
        if not png_b64_str or len(png_b64_str) < 10:
            _stderr_print(f"⚠️  Invalid base64 string (too short): {png_b64_str[:50] if png_b64_str else 'None'}")
            return None
        ...
        _stderr_print(f"❌ Error converting image: {e}")
        _stderr_print(f"   Input preview: {png_b64_str[:100] if png_b64_str else 'None'}")
        return None
