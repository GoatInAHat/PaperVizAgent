...
#
# Modified by PaperVizAgent to use a host-supplied backend and bounded local resources.
...
from .logging import stderr_print as _stderr_print
...
class PaperVizProcessor:
    ...
    async def _run_critic_iterations(self, data: Dict[str, Any], task_name: str, max_rounds: int = 3, source: str = "stylist") -> Dict[str, Any]:
        ...
        for round_idx in range(max_rounds):
            ...
            if data.get(f"target_{task_name}_critic_error{round_idx}"):
                data["critic_stop_reason"] = "invalid_response"
                break
    ...
    async def process_single_query(
        ...
        """
        _stderr_print(f"[DEBUG] -> Entered process_single_query for candidate {data.get('candidate_id', 'N/A')}")
        ...
        if exp_mode == "vanilla":
            _stderr_print(f"[DEBUG] Starting Vanilla Agent processing...")
            ...
            _stderr_print(f"[DEBUG] Starting Retriever Agent processing...")
            data = await self.retriever_agent.process(data, retrieval_setting=retrieval_setting)
            _stderr_print(f"[DEBUG] Starting Planner Agent processing...")
            data = await self.planner_agent.process(data)
            _stderr_print(f"[DEBUG] Starting Visualizer Agent processing...")
            data = await self.visualizer_agent.process(data)
            ...
            _stderr_print(f"[DEBUG] Starting Planner Agent processing...")
            data = await self.planner_agent.process(data)
            _stderr_print(f"[DEBUG] Starting Stylist Agent processing...")
            ...
            _stderr_print(f"[DEBUG] Starting Critic Agent iterations (max {max_rounds})...")
            ...
            _stderr_print(f"[DEBUG] [dev_full] Starting Retriever Agent processing...")
            data = await self.retriever_agent.process(data, retrieval_setting=retrieval_setting)
            _stderr_print(f"[DEBUG] [dev_full] Starting Planner Agent processing...")
            data = await self.planner_agent.process(data)
            _stderr_print(f"[DEBUG] [dev_full] Starting Stylist Agent processing...")
            data = await self.stylist_agent.process(data)
            _stderr_print(f"[DEBUG] [dev_full] Starting Visualizer Agent processing...")
            data = await self.visualizer_agent.process(data)
            # Use max_critic_rounds from data (if set) or config
            max_rounds = data.get("max_critic_rounds", self.exp_config.max_critic_rounds)
            _stderr_print(f"[DEBUG] [dev_full] Starting Critic Agent iterations (max {max_rounds})...")
            data = await self._run_critic_iterations(data, task_name, max_rounds=max_rounds, source="stylist")
            if "demo" in exp_mode: do_eval = False
        
        elif exp_mode == "dev_polish":
            _stderr_print(f"[DEBUG] Starting Polish Agent processing...")
            ...
            _stderr_print(f"[DEBUG] Starting Retriever Agent processing (solo)...")
            data = await self.retriever_agent.process(data, retrieval_setting=retrieval_setting)
            do_eval = False
    ...
    async def process_queries_batch(
        ...
        async def process_with_semaphore(doc):
            _stderr_print(f"[DEBUG] -> Acquring semaphore for candidate {doc.get('candidate_id', 'N/A')}", flush=True)
            async with semaphore:
                _stderr_print(f"[DEBUG] -> Semaphore acquired for candidate {doc.get('candidate_id', 'N/A')}", flush=True)
        ...
        _stderr_print(f"[DEBUG] Created {len(tasks)} tasks.")
    ...
    async def evaluation_function(
        ...
        """
        raise RuntimeError("evaluation is not configured; call run_pipeline with do_eval=False")
