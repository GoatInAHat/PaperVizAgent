...
#
# Adapted for PaperVizAgent-codex: bound generated-code execution in a child process.
...
def _worker(code_text: str, connection: Any) -> None:
    try:
...
def execute_plot_code(code_text: str, timeout_seconds: float = 30) -> str | None:
    receiver, sender = multiprocessing.Pipe(duplex=False)
    process = multiprocessing.get_context("spawn").Process(target=_worker, args=(code_text, sender))
    process.start()
    sender.close()
