# papervizagent — Hermes plugin

Generated by toolfactory from `plugin.json` and `dev.toolfactory/ops.json`.
`papervizagent_hermes/` is a Hermes directory plugin (`plugin.yaml` + `__init__.py`) and a Python
package at the same time, so the same code loads from a checkout and from PyPI.

## Validate

```bash
hermes plugins doctor hosts/hermes/papervizagent_hermes --ci
```
...
hermes plugins enable papervizagent
```

Developing against this checkout, without publishing a package first:

```bash
hermes plugins install file://<repo>#hosts/hermes/papervizagent_hermes --force
```
...
Remove it cleanly with `hermes plugins remove papervizagent` (aliases `rm`,
...
All of them register into the `papervizagent` toolset.
