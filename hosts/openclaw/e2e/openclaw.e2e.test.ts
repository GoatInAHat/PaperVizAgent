...
describe("papervizagent-codex in a real OpenClaw agent turn", () => {
  ...
  it("the model calls status and the tool's result reaches the reply", async () => {
    const turn = JSON.parse(
      await oc([
        "agent", "--local", "--agent", "main", "--session-id", `e2e-${Date.now()}`,
        "--model", MODEL, "--thinking", "off", "--timeout", "120", "--json",
        "--message",
        "Call the tool named status. Reply with only the exact text returned by that tool.",
      ]),
    );
