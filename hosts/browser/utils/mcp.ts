...
/** Where `papervizagent-codex mcp --http` listens by default. */
...
/**
 * Pair with `<url>#<token>`, the string `papervizagent-codex mcp --http --pair` prints. The
...
export async function post(request: McpRequest, endpoint?: string): Promise<string> {
  ...
  try {
    ...
    throw new Error(
      `${url} refused the connection: start \`papervizagent-codex mcp --http\`, and on Chromium make sure the browser is 144 or newer (Local Network Access exempts an extension that holds the host permission).`,
      { cause },
    );
  }
