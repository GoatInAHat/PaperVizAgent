# papervizagent — browser extension
...
tool's own kernel: the background worker POSTs stateless MCP to `papervizagent mcp --http` on
...
destructive. Run `papervizagent mcp --http --pair` and paste the `<url>#<token>` it prints into the
