# papervizagent-codex — browser extension
...
tool's own kernel: the background worker POSTs stateless MCP to `papervizagent-codex mcp --http` on
...
destructive. Run `papervizagent-codex mcp --http --pair` and paste the `<url>#<token>` it prints into the
