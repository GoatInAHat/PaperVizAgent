...
Asset type: landscape raster methodology diagram for a scientific paper or slide
Primary request: Create ONE new diagram from text only, following the complete authoritative specification below. No reference image or editing target exists. Any wording about correcting or preserving a current rendering refers to desired content and style only; realize the final specified state from scratch. Do not render element IDs, Markdown syntax, source metadata, caption, or these instructions in the figure. Use a wide landscape canvas, with the complete pipeline comfortably contained and readable. Exactly three passage cards must be visible.
