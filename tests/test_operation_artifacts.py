...
from papervizagent_codex.ops import InferInput, infer
from papervizagent_codex.toolfactory import Context
...
async def test_infer_preserves_provider_image_format(tmp_path, format, suffix):
    ...
    raw = buffer.getvalue()
    with patch('papervizagent_codex.ops.Backend') as cls:
...
def test_status_hands_safe_native_overrides_to_host(tmp_path):
    import json
    from papervizagent_codex.ops import StatusInput, status
    ...
    result = status(StatusInput(native=['llm', 'vlm']),
                    Context(config={'papervizagent_codex_config': str(config)}, data_dir=tmp_path))
